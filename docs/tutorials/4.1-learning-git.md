...
also want to know the essential command `git status`. This will tell
you at any given time which of the three commands above you 
...
changes, instead we edited directly on the main branch.
This is not the "best practice" when working on a collaborative 
project, but when you are working alone it can often be fine, and 
it is the simplest framework for learning. 
