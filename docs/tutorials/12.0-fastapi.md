...
This tutorial build upon completion of the instructions in notebook 12.0 
so you should first complete that assignment.
</div>
...
server) and it will serve to `localhost:8000`, which we can view in our brower.
When we way this result will be routed to `/`, what we really mean is it will 
be routed to localhost:8000`/`.


### Running the app
To run our app we now just need to call our server tool and point it to the
corrent entry point in our code where the app exists. This is a similar 
syntax to when we pointed to a specific function in the setup.py file to define
a CLI entry point.
...
localhost:8000. You should see a single JSON result showing "message",
"Hello World". Success. Your Python function is writing its results to 
this web address. 


### So what?
In this simple example we can see that the <i>returned value</i> of the 
function `root()` is printed to the webpage on the server at the `/` 
(root) path. Very simple, right? But why is this useful? Well, following 
...
## RESTful API
Let's try adding support for arguments to our function. Here you simply just
add arguments to the function being served at the endpoint. Our root function
now takes an optional `name` argument with a default value of 'person'.
...
browser update automatically. Now try modifying the URL to enter an 
argument by visiting [localhost:8000/?name=Phylo](localhost:8000/?name=Phylo).

### Serving JSON data
Let's extend our script with another example. In this case we will return 
...
commonly web sites/servers will use a different kind of database, like an
SQL-type, which if faster for getting individual results from a very large
database. But that is beyond the scope of our current interest. Add code to
your script so it looks like the example below:
...
After making these changes and saving the file, visit 
[localhost:8000/iris](localhost:8000/iris) to see all of the data records,
 and also try entering arguments such as 
[localhost:8000/iris?species=Iris-setosa](localhost:8000/iris?species=Iris-setosa)
to have only a subset of records returned that match your query.
...
### Backends, webservers, I/O, and apps
It is the basis for beginning to develop a web API, similar to the REST
APIs we learned about in a previous tutorial for accessing data from online
...
need to run it yourself -- but you are welcome to. I think its worth 
walking through so you have an idea of the steps involved with making your API 
...
Unfortunately this tool cannot be installed using conda. The instructions here
are quite similar to those from the example in their docs. You will need to 
create a heroku login on the website. It is all free. (Again, this is optional
for now.)
...
to run our server, this simply contains the command we called to run uvicorn; 
...
Then we need to create a runtime.txt file that simply says which version of 
Python to use:
```bash
python-3.7.10
```
...
git add -A                                # lazy method to stage all files
